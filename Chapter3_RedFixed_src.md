...
- OpenIDS (HIGH-confidence anchor, MEDIUM projection): $19.9K (2024) → ~$16.8K (2026, base) → ~$12.1K (2030, base) if community iteration continues. Crosses <$15K around 2027 (base; 2026 optimistic, 2029–2030 conservative); <$10K is a ~2028 optimistic / ~2032 base prospect — not a base-case-2020s certainty. [[RUpdate (J. Kim, developer correspondence, 2026): OpenIDS2 (2025) is confirmed at ~$4,000, a one-year redesign step-change that crosses <$15K, <$10K and <$5K at once, so the gradual-decline scenarios below are superseded as a capital story. Capital is no longer the binding DIY constraint; fidelity and length are (§4.1).R]]
...
Interpretation: base case <$15K by ~2027; ~$12K by 2030. Conditional on sustained community iteration, stable supply, and continued interest. Reference points: <$15K ≈ a used lab HPLC; <$10K ≈ a modern benchtop PCR machine. Quality caveat: these are capital-cost projections and say nothing about synthesis quality (error rate, length). Quality bottlenecks could require R&D and slow cost decline. [[RUpdate (J. Kim, correspondence, 2026): OpenIDS2 (2025) is developer-confirmed at approximately $4,000, achieved by redesigning most components for in-house fabrication. This is a v1→v2 step-change, not community iteration, and it undercuts every scenario above in a single year, so the annual-decline model is superseded as a capital-cost story. The quality caveat now becomes the headline: a working open build is already sub-$5K, so what limits the DIY route is fidelity and length, not price. Two caveats: the $4,000 buys a lower-capability instrument (short oligos, ~56% full-length for a 15-mer), and Kim attributes that short-oligo ceiling to the inkjet architecture's moisture sensitivity rather than to DIY phosphoramidite synthesis in general (a column-based DIY build could, in his estimate, reach ~100 nt; Ch.1 §2.1). International purchasing can roughly double individual component costs.R]]

[[R**The successor build has already crossed these thresholds, which reframes what the projection means.** OpenIDS2 (Kim, Kim & Bang, 2025) was built for approximately $4,000, a figure the developer confirms from the itemised component list, with the caveat that international purchasing can roughly double individual components such as the printhead (J. Kim, correspondence, 2026). The <$15K and <$10K milestones the base case projects for the late 2020s are therefore already met by an existing, published DIY instrument in 2025, achieved not by a falling cost curve on the v1 design but by redesigning most components for in-house fabrication. The governance implication is a decoupling of cost from capability: OpenIDS2 is cheaper yet no more capable, still producing short oligos (~56% full-length for a 15-mer, Ch.1 §2.1). Capital cost is thus no longer the binding barrier for OpenIDS-class devices; capability (usable length and fidelity) and, per the developer, the surrounding laboratory infrastructure are. The v1 cost trajectory above is retained as an illustration of decline on a fixed design, but the operative fact is that the DIY route is already inexpensive and the constraint has moved to what the device can make.R]]
...
OpenIDS <$15K: most likely ~2027 (base, MEDIUM); range 2026–2030 across scenarios. <$10K: ~2028 optimistic, ~2032 base (MEDIUM-LOW) — not a base-case-2020s event. Electrochemical: TRL 4 ~2027–2029 if R&D accelerates (LOW); cost undefined until then. Enzymatic DIY: unlikely before ~2032–2035 (VERY LOW); remains service/commercial. [[RThe DropSynth developer's estimate is consistent: long, low-error enzymatic oligos are ~5–10 years out commercially and longer for DIY, and enzymatic (not chemical) is the DIY route he expects to mature first (C. Plesa, interview 2026). The OpenIDS developer independently makes the same structural point: because enzymatic synthesis needs no acids, organic solvents, or high-pressure gases, it removes exactly the laboratory-infrastructure barrier that constrains chemical DIY synthesis, so in principle it could become feasible in a home setting once suitable reagents are available (J. Kim, correspondence, 2026). Two developers from different chemistries thus converge on enzymatic as the DIY route to watch, which supports monitoring reagent access (engineered TdT, modified nucleotides) rather than instruments as the enzymatic control point.R]] [[RMAS 2.0: no forecast — a ~€150–170K instrument (~€200–300K loaded) with a single anchor and no trajectory (§4.4).R]]
