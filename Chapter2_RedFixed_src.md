...
**Scenario summary.** Heavily constrained: commercial benchtop, enzymatic benchtop, enzymatic service (domestic). Moderately constrained: OpenIDS and MAS 2.0 (if classified), DropSynth (inputs screened). Weakly constrained: electrochemical, OpenIDS/MAS 2.0 (if exempt), extra-jurisdictional service. Residual vulnerabilities (R1): (1) legacy/used devices without on-device screening; (2) unresolved DIY classification; (3) extra-jurisdictional providers; (4) reagent/solvent substitutability. [[RThe DropSynth input-screening perimeter is a real chokepoint, not a nominal one: a DropSynth practitioner confirms usable assembly requires high-fidelity array oligos, and only a few vendors (Twist, Agilent, Dynegene) reliably clear the ~1-in-500-bp error floor (Ch.1 §2.6). Low-fidelity electrochemical-array oligos (GenScript/CustomArray, LinkZill) produce frameshift-dominated garbage in assembly, so the workflow is pushed onto exactly the centralised, screenable providers a provider mandate would reach. The developer adds a second, independent reason it is low-concern: DropSynth is a massively-multiplexed library method and, in his words, "terrible" for building one specific sequence, so any standard assembly method beats it for a single sequence of concern, and it also needs about 70 nt of every input oligo for the process itself, which rules out short DIY oligos on length alone (C. Plesa, interview 2026). DropSynth's low residual concern therefore rests on two legs, the screened-input perimeter and its unsuitability for single-SOC production, not on fidelity alone. Empirically, the one startup known to have fed self-made oligos into DropSynth (Instance Bio) failed on error rate, so the screened-input perimeter holds in practice, not just in principle.R]]
...
1. DIY-systems classification (OpenIDS, MAS 2.0, electrochemical). Unresolved whether open-source/DIY tools fall under a device mandate. → Define scope explicitly or establish an alternative oversight pathway. [[R(For MAS 2.0, the high capital and small builder population mean classification is lower-urgency than for cheap-and-proliferating routes, but the specialty-amidite/DMD supply signature offers a secondary handle — Ch.1 §2.2.)R]] [[R(For OpenIDS specifically, the developer notes the complete build is public on GitHub and reproducible without contacting him, so a device mandate written against the open design is largely unenforceable and silent builds are unobservable. The governance-reachable handles are the wet-lab inputs (corrosive acids, organic solvents, pressurised gases) and purchased reagents/oligos, not the open plans. He also judges the near-term capability limit to be inkjet-specific: a column-based DIY build could reach ~100 nt, which would move the residual-gap object from 15–30 nt oligos toward assembly-relevant lengths (Ch.1 §2.1; J. Kim, correspondence, 2026).)R]] [[RA scope note the OpenIDS developer sharpens: the existing open instruments make only short oligos (~15–30 nt), but he attributes that ceiling to the inkjet architecture and its moisture sensitivity, not to a limit on DIY synthesis, and estimates that a DIY build on conventional column chemistry could reach ~100 nt with assembly to longer molecules (J. Kim, correspondence, 2026; Ch.1 §2.1). No such column-DIY instrument has been published, so this is a forward risk rather than a present capability, but it means a classification scope written only around today's short-oligo open hardware could be outrun by a column-based open build; any definition should be drafted by capability and architecture, not by naming current devices. He also locates the practical barrier in laboratory infrastructure for handling acids, solvents, and pressurised inert gases rather than in the build, and those are commodity facilities that no input-side control reaches.R]]
