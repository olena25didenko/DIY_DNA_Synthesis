...
**Forensically relevant build detail.** The published protocol omits the capping step. Because the diagnostic G→A substitution signature of column phosphoramidite chemistry is capping-driven (Masaki, Onishi & Seio, 2022; see Chapter 4), OpenIDS oligos are expected to carry a distinguishable error phenotype relative to standard capped column synthesis. [[RThe developer confirms capping was deliberately omitted in both OpenIDS and OpenIDS2, to cut reagents and the associated pumps, tubing, control code, and space, and that a double-coupling step (re-attempting the addition when the first coupling fails; cf. Agilent patent US10072261B1) was used in its place. He expects builders to prefer omitting capping wherever coupling efficiency is high enough, which supports treating capping omission as a characteristic, though not universal, DIY signature (J. Kim, correspondence, 2026; see Chapter 4).R]]
...
[[R**The short-oligo ceiling is inkjet-specific, not a general DIY limit (J. Kim, OpenIDS lead developer; correspondence, 2026).** The developer confirms the ~15–30 nt ceiling reflects moisture control, not a fundamental barrier to open synthesis. OpenIDS delivers phosphoramidite as picolitre inkjet droplets whose very high surface-area-to-volume ratio makes coupling extremely sensitive to trace atmospheric moisture, which is hard to exclude in a DIY setting; this was a deliberate trade-off in favour of massive parallelism. He notes that a DIY synthesizer built on conventional column-based phosphoramidite chemistry (the IDT/GenScript architecture) would face a far easier moisture problem and, in his estimate, could synthesize arbitrary sequences of around 100 nt, then assemble those into longer molecules. This reframes the residual DIY route: the 15–30 nt figure characterises OpenIDS's inkjet design specifically, and should not be generalised to all DIY phosphoramidite synthesis, where a column build could plausibly reach assembly-relevant lengths. No such DIY column instrument has been published; this is a stated capability estimate, not a demonstrated one.R]]

[[R**The binding barrier is laboratory infrastructure, not the build.** The developer reframes the accessibility barrier away from fabrication skill. OpenIDS was designed so that someone without professional hardware or software training could build it in a few months, and he judges that a typical science or engineering researcher could now build OpenIDS2 within weeks, given AI-assisted CAD, circuit, and code design. The greater bottleneck, in his account, is obtaining the laboratory infrastructure to safely handle corrosive acids, organic solvents, and pressurised inert gases (nitrogen or argon), not building the synthesizer. The expertise score below is retained but reinterpreted: the barrier is a facilities-and-materials-handling question (fume hood, solvent and acid handling, compressed inert gas) rather than a fabrication-skill question. Capital for OpenIDS2 is confirmed at approximately $4,000 from the itemised component list, with the caveat that international purchasing can roughly double individual component prices such as the printhead (J. Kim, correspondence, 2026).R]]
...
[[R**Input-oligo fidelity sets the ceiling (C. Plesa, DropSynth co-inventor, University of Oregon; interview, 2026).** A DropSynth practitioner confirms the binding constraint is input-oligo quality, not the assembly chemistry, which is now optimised to add few errors of its own. Input errors pass through and compound: if the input oligos are X% perfect and a gene assembles from N of them, the product is at best X^N perfect, and the errors are dominated by deletions (frameshifts) that most biological systems tolerate poorly. So DropSynth carries a hard fidelity floor set by its oligo source. Oligos from electrochemical-deprotection array vendors (GenScript/CustomArray, LinkZill) have deletion rates too high to use; they produce garbage in assembly, consistent with the high electrochemical deletion rate in Ch.4 §4.1(d) (~1.35%/nt reported; 0.835%/nt in our reproduction). Only a few vendors reliably clear the roughly 1-in-500-bp array-oligo error floor: Twist, Agilent, and Dynegene. Fishing a single perfect gene out of a dirty pool by dial-out PCR runs about 70% success for products under ~1 kb, and only when perfects are present and observable at the sequencing depth and diversity bottleneck; selection helps less than assumed (transformation bottlenecks, cheating cells, hitchhikers/parasites). For governance this tightens the "inherits the perimeter" reading rather than loosening it: usable DropSynth output needs high-fidelity commercial array input from a short, screenable vendor list.R]]

[[RThe interview adds a structural barrier separate from fidelity. DropSynth consumes about 70 nt of every input oligo for barcodes and assembly overlaps before any payload, so a 150-mer leaves roughly 80 nt of usable sequence, and sub-70-nt DIY oligos (OpenIDS 15–30 nt, electrochemical 13–17 nt) cannot feed the process at all, regardless of error rate. The three high-fidelity vendors (Twist, Agilent, Dynegene) reach ≥230 nt at ≤1-in-500 by inkjet phosphoramidite, a length-and-fidelity combination no DIY route approaches. On accessibility, Plesa reports undergrads run the protocol after one or two demonstrations; the differentiator is library-handling experience, so the 8/10 score holds for a standing start but is lower for a trained library worker. His own biosecurity read: DropSynth is a massively-multiplexed library method and "terrible" for building one specific sequence, so it is the wrong tool for a single sequence of concern (see Ch.2).R]]

[[REmpirical precedent backs the input-fidelity wall. Instance Bio, a Bay Area startup, tried feeding self-synthesised oligos into DropSynth and it failed on error rate, and Plesa knows of no lab that has run DropSynth on anything but Twist, Agilent, or Dynegene. Other DIY oligo sources (Doohee Kim's 3D-printed inkjet = OpenIDS, DNA Script's enzymatic spotter, and one or two others in Georgia and China) are all "too short and too error-prone," either flaw disqualifying on its own. The development roadmap points the same way, toward more multiplexing rather than single genes: a forthcoming preprint moves to ~12,000 genes per reaction, next year targets 100,000+, and DropSynth-Gold (Golden-Gate emulsion assembly, bioRxiv 2026) extends length. None of that helps someone who wants one specific sequence.R]]
...
| OpenIDS (inkjet) | moderate | moderate–high (must build the device) | low | no | 6 | Build from 3D-printed parts + Arduino/Pi + printhead, then run wet chemistry with calibration; designed as "non-expert buildable." [[RDeveloper reframes the barrier: the build is weeks-accessible with AI-assisted design, and the real constraint is lab infrastructure for corrosive acids, organic solvents, and pressurised inert gases (J. Kim, 2026). Score retained; barrier reinterpreted as facilities/materials-handling, not fabrication skill.R]] |
