...
| DropSynth (emulsion assembly) | 4 | ~$3.4K bead pool (~200 rxns) | ~1–3 kb assembled; ~4% perfect in the original method (Plesa et al., 2018), improved ~5-fold to a median ~22–28% perfect in DropSynth 2.0 (Sidore et al., 2020) | ~$1–2/gene | Medium → Low | Inherits provider screening through its commercial oligo-pool input; the assembly step is untracked. Screening depends on the input oligos being ordered through a participating provider |
...
|---|---|---|---|---|---|---|
| OpenIDS / OpenIDS2 (inkjet) | 5 | ~$19.9K (v1); ~$4K (v2, 2025) | ~15–30 nt (inkjet-limited; column DIY could reach ~100 nt, developer est.) | ~$1–5/seq | Provider screening on any purchased reagents or oligos only | Column phosphoramidite; capping omitted → predicted suppressed G→A + elevated n−1 (unmeasured: no OpenIDS reads exist; DIY signatures expected heterogeneous). Core residual DIY route; not gene-assembly-ready |
...
Two open synthesizers are reproducible, and both are academic instruments with limited output. OpenIDS, an open-source inkjet system costing approximately $19,900, produces usable oligonucleotides of roughly 15 to 30 nucleotides (Kim, Kim & Bang, 2024). At 98% per-step coupling efficiency a 30-mer is full-length only about 55% of the time, and the second-generation build measured approximately 56% full-length for a 15-mer (Kim, Kim & Bang, 2025). The second-generation build was assembled for about $4,000, so cost is no longer the barrier for this class of device (Kim, Kim & Bang, 2025; J. Kim, correspondence, 2026).

A 2026 exchange with the OpenIDS lead developer adds an important qualification that this short-oligo picture should not be over-generalised. The 15 to 30 nucleotide ceiling is specific to the inkjet architecture, which delivers reagent as picolitre droplets that are highly sensitive to atmospheric moisture and hard to keep dry in a DIY setting; it was a deliberate trade-off in favour of massive parallelism, not a fundamental limit of open synthesis (J. Kim, correspondence, 2026). The developer's view is that a DIY synthesizer built on conventional column-based chemistry, the architecture used by established providers, would face a far easier moisture problem and could plausibly reach around 100 nucleotides, a length that would support assembly into longer molecules. No such DIY column instrument has been published, so this is a stated capability estimate rather than a demonstrated one, but it means the residual DIY concern is better framed around the column architecture that no open project has yet built than around the inkjet instruments that exist. The same developer reframes the barrier to building one: with AI-assisted design the build itself is now a matter of weeks for a typical researcher, and the real constraint is obtaining laboratory infrastructure to handle corrosive acids, organic solvents, and pressurised inert gases safely, rather than fabrication skill.
...
None of the remaining routes alters this picture. Electrochemical synthesis rests on a single data-storage proof-of-concept at technology readiness level 3, with no independent build and no commercial instance among the 34 manufacturers inventoried (Xu et al., 2021). Enzymatic synthesis is available in DIY form only for data storage, where it produces stochastic homopolymers (Lee et al., 2019); the defined-sequence route depends on an engineered enzyme and bespoke nucleotides that are proprietary and take years to develop (Palluk et al., 2018). DropSynth does offer inexpensive gene assembly at around $3,400 for the bead pool, but it consumes a commercial microarray oligonucleotide pool and therefore inherits the screening perimeter through its own input (Sidore et al., 2020; Plesa et al., 2018). A 2026 interview with DropSynth's developer adds two constraints (C. Plesa, 2026): the process consumes about 70 nucleotides of each oligo for barcodes and overlaps before any payload, so short DIY oligos can't feed it regardless of error rate; and DropSynth is a massively-multiplexed library method that is, in his words, "terrible" for building one specific sequence, making it the wrong tool for a single sequence of concern. One startup (Instance Bio) that tried feeding self-made oligos into DropSynth failed on error rate, and no lab is known to have run it on anything but the three high-fidelity vendors. A 2026 survey identified no working homebrew synthesizer beyond these academic platforms.
...
Plesa, C. (2026). DropSynth developer interview (DropSynth co-inventor, University of Oregon), conducted by O. Didenko for ERA/IBBIS. Source for the DropSynth input-length (~70 nt process floor), oligo-quality (three high-fidelity vendors: Twist/Agilent/Dynegene), cost, expertise, and single-sequence biosecurity points.

Kim, J. (2026). OpenIDS developer correspondence (OpenIDS/OpenIDS2 lead developer), with O. Didenko for ERA/IBBIS. Source for the inkjet-specific short-oligo ceiling and the column-DIY ~100 nt capability estimate, the laboratory-infrastructure barrier, the OpenIDS2 ~$4,000 cost, the deliberate capping omission (double-coupling per Agilent US10072261B1), the absence of shareable OpenIDS sequencing data, and the greater heterogeneity of DIY versus commercial synthesis signatures.
